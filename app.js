...
vk.setOptions({ token: 'f278cb44e23c74ed9e99e12bf26621559e98a6a2ae1276e7c6747ea73d590755a223970dcaf85119039e5', pollingGroupId: 185885672 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[club185885672\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club185885672\|(.*)\]/ig, '').trim();
	...
	if(!users.find(x=> x.id === message.senderId))
	{
		...
		message.send(`🎉 ➾ *id`+message.senderId+`, вы успешно зарегистрировались!\n📝 ➾ "Помощь" - список команд.\n📝 ➾ Чтобы не пропустить обновление проекта вы должны быть подписаны на нашу группу: [reportpagebot|RandomBot]`)
...
cmd.hear(/^(?:генпасс|genpass|generatepassword|сгерерирыватьпароль)\s?([^]+)?/i, async (message) => {
	request("https://reportest229.000webhostapp.com/api.php?key=nyO9KYDv&method=genpass").then(res => {
		message.send("[😅] Держи пароль: "+ res);
	})
});

cmd.hear(/^(?:генчисло|Сгенерировать число)\s?([^]+)?/i, async (message) => {
	request("https://reportest229.000webhostapp.com/api.php?key=nyO9KYDv&method=number").then(res => {
		message.send("[😅] Держи цифру: "+ res);
	})
});

cmd.hear(/^(?:eval)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId !== 492672295) return;
...
🔥 Тест api {NEW} (RandomApi 0.1 beta):
генпасс - Генерирует пароль
генчисло - Генерирует число от 1 до 100

🆘 Репорт [фраза] - ошибки или пожелания
Если вы нашли ошибку то свяжитесь с [reportpage|Админом]`);
});
...
cmd.hear(/^(?:помощь)\s(.*)$/i, async (message, bot) => {
	...
	if(message.args[1] === 'генпасс')
	{
		return message.send(`Команда "Генпасс" генерирует рандомные знаки/символы для вашего пароля.`);
	}
	
	if(message.args[1] === 'генчисло')
	{
		return message.send(`Команда "Генчисло" генерирует любое число от 1 до 100.`);
	}
});
...
cmd.hear(/^(?:профиль)$/i, async (message, bot) => {
	...
	if(message.user.admlvl) text += ` 😎 Статус: Администратор\n`;
	return bot(`твой профиль:\n${text}`);
});

cmd.hear(/^(?:кикнуть)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => { 
	let user = users[message.senderId(message.user)];
	if(message.$from.type == 'user') return message.send(`⚠ ➾ Команда работает только в беседах!`);
 	if(message.user.admlvl -= 1) return message.send(`⚠ ➾ Вы не Администратор`);
	...
	if(message.$match[4]) { 
		...
		screen_name: message.$match[4] 
			...
			if(res.object_id == 492672295) return message.reply('⚠ ➾ Отказ'); 
		...
		if(message.$match[3] == 492672295) return message.reply('⚠ ➾ Отказ'); 
...
cmd.hear(/^(?:репорт|реп|rep|жалоба)\s([^]+)$/i, async (message, bot) => {
	...
	vk.api.messages.send({ user_id: 492672295, forward_messages: message.id, message: `Player id: ${message.user.uid}` }).then(() => {
...
if(message.senderId !== 492672295| message.senderId !== 492672295) return; 
